...
def _extract_factor_signal(bundle: HistoricalInfoBundle, forward_return_pct: float) -> dict[str, float]:
    ...
    event_count = float(len(bundle.dated_events))
    ...
    target: dict[str, float] = {
        ...
        "event_risk": min(1.0, event_count / 3.0),
    ...
    if forward_return_pct > 0:
        ...
        target["event_risk"] = min(1.0, target["event_risk"] + 0.2)
...
def evaluate_trade_decision_step(
    ...
    direction = 1.0 if decision.action == expected_action else 0.0
    ...
    calibration = _clamp01(1.0 - calibration_error)
    ...
    if decision.action == TradeAction.HOLD:
        ...
        target_size = 0.05
    sizing = _clamp01(1.0 - abs(float(decision.size_pct) - target_size))
    ...
    factor_relevance = _clamp01(factor_hits / max(1, len(expected_top)))
...
def evaluate_factor_weight_ranking(
    ...
    ranking_score = _clamp01(overlap / max(1, len(top_expected)))
    ...
    if weight_map:
        ...
        weight_quality = _clamp01(1.0 - l1_error)
    else:
        weight_quality = 0.0

    low_signal = {name for name, value in target.items() if value < 0.35}
    noisy_detect = set(ranking.noisy_metrics)
    noise_score = _clamp01(len(noisy_detect & low_signal) / max(1, len(noisy_detect))) if noisy_detect else 0.0
    ...
    score = 0.45 * ranking_score + 0.35 * weight_quality + 0.1 * noise_score + 0.1 * rationale_score
    notes: list[str] = []
    if ranking_score < 0.4:
        notes.append("Top-ranked factors did not match outcome-relevant factors.")
    if weight_quality < 0.4:
...
def evaluate_post_trade_reflection(
    ...
    if was_correct:
        decision_label_score = 1.0 if label in {"good", "strong"} else 0.5 if label == "mixed" else 0.0
    else:
        decision_label_score = 1.0 if label in {"bad", "weak"} else 0.5 if label == "mixed" else 0.0
    ...
    if calibration_good:
        confidence_score = 1.0 if any(token in confidence_text for token in ("appropriate", "calibrated", "reasonable")) else 0.6
    else:
        confidence_score = 1.0 if any(token in confidence_text for token in ("too high", "too low", "overconfident", "underconfident")) else 0.2
    ...
    if size_gap <= 0.2:
        size_score = 1.0 if any(token in size_text for token in ("appropriate", "reasonable", "right-sized")) else 0.6
    else:
        size_score = 1.0 if any(token in size_text for token in ("too large", "too small", "oversized", "undersized")) else 0.2

    process_quality = reflection.process_quality.lower()
    if was_correct:
        process_score = 1.0 if process_quality in {"good", "strong"} else 0.6 if process_quality == "mixed" else 0.2
    else:
        process_score = 1.0 if process_quality in {"bad", "weak"} else 0.6 if process_quality == "mixed" else 0.2
